...
database:set(bot_id.."EGYBOTSS:Tag:Admins:"..msg.chat_id_,true)
send(msg.chat_id_, msg.id_,"⚠️┇تم تفعيل تاك الادمنيه")
end
if text and text == "تعطيل تاك الادمنيه" and Owner(msg) then 
database:del(bot_id.."EGYBOTSS:Tag:Admins:"..msg.chat_id_)
send(msg.chat_id_, msg.id_, "⚠️┇تم تعطيل تاك الادمنيه")
end

if text == 'تاك الادمنيه' or text == "تاك للادمنيه" or text == "وين الادمنيه" then
if database:get(bot_id.."EGYBOTSS:Tag:Admins:"..msg.chat_id_) then 
...
local t = "\n⚠️┇المستخدم ~ ["..User_id .."] يتاك الادمنيه \n\n"
k = 0
for i,v in pairs(data.members_) do
if bot_id ~= v.user_id_ then 
k = k + 1
local username = database:get(bot_id.."EGYBOTSS:User:Name"..v.user_id_)
if username then
t = t..""..k.."- ([@"..username.."])\n"
else
t = t..""..k.."- (`"..v.user_id_.."`)\n"
